...
class UNet2D:
	...
	def setup(imSize, nChannels, nClasses, nOut0, featMapsFact, downSampFact, kernelSize, nExtraConvs, stdDev0,
		...
		def down_samp_layer(data, index):
			drop = [0,0.2,0.2,0.35]
			regularizer = tf.contrib.layers.l1_regularizer(scale=0.00008)
			with tf.variable_scope('ld%d' % index):
				...
				ldXWeightsShortcut = tf.get_variable(initializer=tf.contrib.layers.variance_scaling_initializer(mode='FAN_IN'),
				...
				bn = tf.layers.dropout(bn, drop[index], training=UNet2D.tfTraining)
			...
			with tf.variable_scope('lu%d' % index):
				...
				for i in range(nExtraConvs):
					cv = tf.nn.leaky_relu(tf.nn.conv2d(cv, luXWeightsExtra[i], strides=[1, 1, 1, 1], padding='SAME'),
										  name='conv2Extra%d' % i)
				cv = tf.layers.dropout(cv, drop[index], training=UNet2D.tfTraining)
				return cv
	...
	def train(imPath, validPath, testPath, logPath, modelPath, pmPath, nTrain, nValid, nTest, restoreVariables, nSteps, gpuIndex,
		...
		for i in range(nSteps):
			...
			for j in range(batchSize):
				...
				for iChan in range(nChannels):
					image[:,:,iChan]= Train[permT[jT + j], :, :, 0, iChan] * fCont + fBrig
					if iChan == 0:
						image[:,:,iChan] = skimage.filters.gaussian(image[:,:,iChan],sigma= sigmas[math.floor(4*np.random.rand())])
			...
			for j in range(batchSize):
				image = np.zeros((imSize, imSize, nChannels))
				image[:, :, 0] = skimage.filters.gaussian(Valid[permV[jV + j], :, :, 0, 0], sigma= sigmas[math.floor(4*np.random.rand())])* fCont + fBrig
...
if __name__ == '__main__':
	# logPath = 'D:\\LSP\\UNet\\LPTCGSdapiFakeAug4-80-32\\TFLogs'
	scriptPath = os.path.dirname(os.path.realpath(__file__))
	modelPath = scriptPath + '\\LPTCGS_DNA_GaussianAug4-80-32'
	# pmPath = 'D:\\LSP\\UNet\\LPTCGSdapiFakeAug4-80-32\\TFProbMaps1'

	# ----- for training -----

	# imPath = 'D:\\LSP\\cycif\\LPTCGSdapilaminRTAug64'
	# validPath = 'D:\\LSP\\cycif\\LPTCGSdapilaminRTAug64\\valid'
	# testPath = 'D:\\LSP\\cycif\\LPTCGSdapilaminRTAug64\\test1'
	#
	# UNet2D.setup(256, 1, 3, 80, 2, 2, 3, 0, 0.03, 4, 32)  # 64 0.0001 4
	# UNet2D.train(imPath, validPath, testPath, logPath, modelPath, pmPath,  0, 0, 3, True, 0, 1, 2)
	# UNet2D.deploy(testPath, 228, modelPath, pmPath, 0, 1)
	...
	UNet2D.singleImageInferenceSetup(modelPath, GPU)
	...
	contours = np.uint8(255 * UNet2D.singleImageInference(cells, 'accumulate', 1))
	contours = resize(contours, (rawVert, rawHorz))
	skio.imsave(args.outputPath + '//' + fileNamePrefix[0] + '_ContoursPM_' + str(dapiChannel) + '.tif',
					  np.uint8(255 * contours), **save_kwargs)
	skio.imsave(args.outputPath + '//' + fileNamePrefix[0] + '_ContoursPM_' + str(dapiChannel) + '.tif',
					  np.uint8(255 * rawI), **append_kwargs)
	del contours
	nuclei = np.uint8(255 * UNet2D.singleImageInference(cells, 'accumulate', 2))
	nuclei = resize(nuclei, (rawVert, rawHorz))
	skio.imsave(args.outputPath + '//' + fileNamePrefix[0] + '_NucleiPM_' + str(dapiChannel) + '.tif',
