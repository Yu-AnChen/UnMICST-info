	...
	def train(imPath, validPath, testPath, logPath, modelPath, pmPath, nTrain, nValid, nTest, restoreVariables, nSteps, gpuIndex,
		...
		for i in range(nSteps):
			...
			for j in range(batchSize):
				...
				for iChan in range(nChannels):
					image[:,:,iChan]= Train[permT[jT + j], :, :, math.floor(6*np.random.rand()), iChan] * fCont + fBrig
			...
			for j in range(batchSize):
				image = np.zeros((imSize, imSize, nChannels))
				image[:, :, 0] = Valid[permV[jV + j], :, :, math.floor(6*np.random.rand()), 0]
...
if __name__ == '__main__':
	...
	# logPath = 'D:\\LSP\\UNet\\LPTCGSdapilaminRTAug4-80-32\\TFLogs'
	scriptPath = os.path.dirname(os.path.realpath(__file__))
	modelPath = scriptPath + '\\LPTCGS_DNA_NES_Aug4-80-32'
	# pmPath = 'D:\\LSP\\UNet\\LPTCGSdapiRTAug4-80-32\\TFProbMaps1'
	...
	#
	# UNet2D.setup(256, 2, 3, 80, 2, 2, 3, 0, 0.03, 4, 32)  # 64 0.0001 4
